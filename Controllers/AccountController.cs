...
namespace Spark.Controllers
{
    public class AccountController : Controller
    {
        ...
        {
            var email = User.Claims.Where(x => x.Type == "UserEmail").FirstOrDefault().Value;
        ...
        public async Task<IActionResult> Profile(ApplicationUser model)
        {
            if (ModelState.IsValid)
            {
                var result = await _accountRepository.UpdatePrfile(model);
                if (result.Succeeded)
                {
                    ViewBag.IsSuccess = true;
                    ModelState.Clear();
                    var dbUser = await _accountRepository.GetUserByEmailAsync(model.Email);
                    return View(dbUser);
                }
